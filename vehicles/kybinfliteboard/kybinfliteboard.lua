...
function init()
  ...
  self.crouchingMovementSettings = config.getParameter("crouchingMovementSettings")
  ...
  --initial state
  self.radioCanToggle = true
  self.radioOn = false
  self.currentTrack = 1
  self.trackSelection = config.getParameter("vehicleConfig.radioTracks")
...
function move()
  ...
  if self.driver then
    if self.crouching then
	 mcontroller.applyParameters(self.crouchingMovementSettings)
	else
...
function controls()
  ...
  if (vehicle.controlHeld("drivingSeat", "AltFire")) then
	  if (self.radioCanToggle) then
		  if self.radioOn == false then
			  self.radioOn = true
			  animator.playSound("headlightSwitchOn")
        ...
        if self.currentTrack > #self.trackSelection then
          self.currentTrack = 1
        end

        animator.setSoundPool("radio", {self.trackSelection[self.currentTrack]})
        animator.playSound("radio", -1)
          ...
          stopRadio()
		    ...
		    self.radioCanToggle = false

		if self.radioOn == true and self.crouching == false then
			vehicle.setLoungeDance("drivingSeat", config.getParameter("radioOnEmote"))
		elseif self.crouching == false then
			vehicle.setLoungeDance("drivingSeat", "balance")
		end
  ...
  if (vehicle.controlHeld("drivingSeat", "special1")) then
    if not self.crouching then
      vehicle.setLoungeDance("drivingSeat", "ollie")
	  self.hoverTargetDistance = config.getParameter("crouchingMovementSettings.hoverTargetDistance")
	  self.danceOverrideTimer = config.getParameter("crouchingMovementSettings.danceOverrideTimer")
	  self.horizontalControlForce = config.getParameter("crouchingMovementSettings.horizontalControlForce")
	  self.jumpVelocity = config.getParameter("crouchingMovementSettings.jumpVelocity")
      self.crouching = true;
    end
  else
    if self.crouching then

	  if self.radioOn == true then
		   vehicle.setLoungeDance("drivingSeat", config.getParameter("radioOnEmote"))
	  else
		   vehicle.setLoungeDance("drivingSeat", "balance")
	  end
